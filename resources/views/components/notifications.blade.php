    ...
    @if(session()->has('notify-saved'))
        ...
        >
            <div class="flex items-center space-x-2">
                ...
                <p class="font-semibold">{{ session('notify-saved') }}</p>
            </div>
        </button>
    @endif
    @if(session()->has('notify-danger'))
        <button 
            type="button"
            class="fixed bottom-8 right-8 z-50 rounded-md bg-red-500 px-4 py-2 text-white transition hover:bg-green-600"
            x-data="{ open: true}"
            x-init="
                setTimeout(() => { open = false}, 3500);
                setTimeout(() => { $refs.successNotification.remove() }, 3800)
        ...
        >
            <div class="flex items-center space-x-2">
                ...
                <p class="font-semibold">{{ session('notify-danger') }}</p>
            </div>
        </button>
    @endif
</div>